...
from profixinvoice.components import SimpleTable, TableWithHeader, PaidStamp
from profixinvoice.models import PDFInfo, Item, Transaction, InvoiceInfo, ServiceProviderInfo, ClientInfo
...
class SimpleInvoice(SimpleDocTemplate):
    ...
    def _invoice_info_data(self):
        if isinstance(self.invoice_info, InvoiceInfo):
            props = [('invoice_id', _('Invoice id')), ('invoice_datetime', _('Invoice date')),
                     ('due_datetime', _('Invoice due date'))]
    ...
    def _build_invoice_info(self):
        ...
        if invoice_info_data:
            self._story.append(Paragraph(_('Invoice'), self._defined_styles.get('RightHeading1')))
    ...
    def _service_provider_data(self):
        if isinstance(self.service_provider_info, ServiceProviderInfo):
            props = [('name', _('Name')), ('street', _('address')), ('city', _('city')), ('state', _('Building')),
                     ('country', _('country')), ('post_code', _('Departament')), ('vat_tax_number', _('Phone')),('email',_('email'))]
    ...
    def _client_info_data(self):
        ...
        props = [('name', _('Name')), ('provider', _('Provider')), ('city', _('city')), ('state', _('Building')),
                 ('country', 'Country'), ('post_code', _('Departament')), ('email', 'Email'), ('client_id', 'Client id')]
    ...
    def _build_service_provider_and_client_info(self):
        if isinstance(self.service_provider_info, ServiceProviderInfo) and isinstance(self.client_info, ClientInfo):
            ...
            table_data = [
                [
                    Paragraph(_('Client Info'), self._defined_styles.get('Heading1')), '',
                    '',
                    Paragraph(_('Project Info'), self._defined_styles.get('Heading1')), ''
                ]
            ]
            table_style = [
                ('SPAN', (0, 0), (1, 0)),
    ...
    def _item_data_and_style(self):
        ...
        self._story.append(
            Paragraph(_('Detail'), self._defined_styles.get('Heading1'))
        )

        item_data_title = (_('Name'), _('Description'), _('Units'), _('Unit Price'), _('Amount'))
    ...
    def finish(self):
        ...
        kwargs = {}
        if self.is_paid:
            kwargs['onFirstPage'] = PaidStamp(9 * inch, .5 * inch)
